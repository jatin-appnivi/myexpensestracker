...
class MonthlyComponent extends Component {
    ...
    submitAddExpenseForm(values) {
        ...
        console.log(values)
        console.log(values.expense['deposit'])
        this.addExpense(values)
    }

    addExpense(values) {
        if(values.expense['deposit'] == "deposit"){
            var is_deposit = "true"
        }
        else{
            var is_deposit = "false"
        }
        fetch(constants.server_base_url + '/expense/addExpense', {
            method: "POST",
    ...
    fetchExpense() {
        fetch(`http://appnivi.tech:8110/expense/fetchAllExpenseOfUser?userCode=${JSON.parse(sessionStorage.getItem('name')).code}`, {
            method: "GET",
            headers: { 'Content-Type': 'application/json' }
        })
            .then((response) => response.json())
            ...
            <>
                ...
                <Modal title="Basic Modal" visible={this.state.isModalVisible} footer={null} onCancel={() => this.showModal(false)}>
                    ...
                    >
                        ...
                        <Form.Item label="Expense or Deposit">
                            <Input.Group compact>
                                <Form.Item
                                    name={['expense', 'deposit']}
                                    noStyle
